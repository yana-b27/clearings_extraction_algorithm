...
class ImageDataset:
    ...
    Methods:
      add_channels(url_summer_image, url_winter_image): Adds channels from summer and winter images to the image dataset.
...
def rasterize_polygons(url_shapefile, url_raster, url_rasterized_polygons):
    ...
    out_raster_ds = memory_driver.Create(
        url_rasterized_polygons, ncol, nrow, 1, gdal.GDT_Byte
    )
...
class ClearingsExtractor:
    ...
    def edge_detector(self, pred_map):
        ...
        bare_lands_without_noise = binary_opening(
            model_bare_lands, skimage.morphology.disk(3)
        )
    ...
    просек, участок № {row_num+1}""")
        ...
        plt.suptitle(
            "Результаты выделения лесных просек под ЛЭП на маске лугов из карты классификации",
            fontsize=20,
            fontweight="bold",
        )
        plt.tight_layout()
...
def find_clearing_algorithm(url_summer_image, url_winter_image, model):
    ...
    pred_map = LandClassificationModel.predict_for_image(
